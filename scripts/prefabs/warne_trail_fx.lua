local assets =
{
    Asset("ANIM", "anim/warne_trail_fx.zip"),
}
...
local function PlayShadowAnim(proxy, anim, scale, flip)
    ...
    inst.AnimState:SetBank("warne_trail_fx")
    inst.AnimState:SetBuild("warne_trail_fx")
...
local function OnRandDirty(inst)
    ...
    inst:DoTaskInTime(0, PlayShadowAnim, "dark"..inst.variation, scale, flip)
end
...
for i = 1, NUM_VARIATIONS do
    local name = "warne_trail_fx"..tostring(i)
    table.insert(prefs, name)
    table.insert(ret, MakeShadowFX(name, i))
end
table.insert(ret, MakeShadowFX("warne_trail_fx", nil, prefs))
prefs = nil
